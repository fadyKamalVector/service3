...
def recursive_group(df, chart_type="bar", level=0):
    ...
    for name, group in df.groupby(col, sort=False):
        ...
        if numeric_cols:
            if chart_type == "bar":
                fig = px.bar(group, x=group.columns[0], y=numeric_cols, title=f"{col}: {name}")
            elif chart_type == "line":
                fig = px.line(group, x=group.columns[0], y=numeric_cols, title=f"{col}: {name}")
            elif chart_type == "pie" and len(numeric_cols) == 1:
                fig = px.pie(group, names=group.columns[0], values=numeric_cols[0], title=f"{col}: {name}")
            else:
                fig = px.bar(group, x=group.columns[0], y=numeric_cols, title=f"{col}: {name}")
...
<body>
    ...
    <form method="get">
        <label>اختر نوع الشارت:</label>
        <select name="chart_type">
            <option value="bar" {{ 'selected' if selected_chart=='bar' else '' }}>Bar</option>
            <option value="line" {{ 'selected' if selected_chart=='line' else '' }}>Line</option>
            <option value="pie" {{ 'selected' if selected_chart=='pie' else '' }}>Pie</option>
        </select>
